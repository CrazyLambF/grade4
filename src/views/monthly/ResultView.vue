<template>
  <div class="page-container result-page">
    <van-nav-bar title="月月练结果" left-arrow @click-left="$router.push('/monthly')" />
    <div class="content" v-if="record">
      <div class="score-card" :style="{ background: scoreGradient }">
        ...
        <div class="score-stats">
          <div class="stat"><span class="stat-value">{{ record.correctCount }}</span><span class="stat-label">答对</span></div>
          <div class="stat-divider"></div>
          <div class="stat"><span class="stat-value">{{ record.totalQuestions - record.correctCount }}</span><span class="stat-label">答错</span></div>
          <div class="stat-divider"></div>
          <div class="stat"><span class="stat-value">{{ record.correctRate }}%</span><span class="stat-label">正确率</span></div>
        </div>
      </div>
      ...
      <div class="detail-list">
        <div v-for="(ans, i) in record.answers" :key="i" class="card detail-item">
          ...
          <div class="detail-answers">
            <p class="your-answer wrong" v-if="!ans.correct">你的答案：{{ ans.userAnswer || '未作答' }}</p>
            <p class="correct-answer">正确答案：{{ Array.isArray(getQuestion(ans.questionId)?.answer) ? (getQuestion(ans.questionId)?.answer as string[]).join('、') : getQuestion(ans.questionId)?.answer }}</p>
            <p class="analysis" v-if="getQuestion(ans.questionId)?.analysis">解析：{{ getQuestion(ans.questionId)?.analysis }}</p>
          </div>
        </div>
      </div>
      <div class="action-buttons">
        <van-button plain type="primary" block round @click="$router.push('/mistakes')">查看错题</van-button>
        <van-button type="primary" block round color="#7B61FF" @click="$router.push('/monthly')">再来一测</van-button>
      </div>
    </div>
...
const scoreGradient = computed(() => {
  ...
  if (s >= 90) return 'linear-gradient(135deg, #52C41A 0%, #73D13D 100%)'
  if (s >= 70) return 'linear-gradient(135deg, #7B61FF 0%, #9D8AFF 100%)'
  if (s >= 60) return 'linear-gradient(135deg, #FAAD14 0%, #FFC53D 100%)'
...
function getQuestion(qId: string) { return allQuestions.value[qId] }
function loadAllQuestions(subject: SubjectType) {
  subjectStore.getSubject(subject).units.forEach(u => u.lessons.forEach(l => l.reviewContent?.exercises.forEach(q => allQuestions.value[q.id] = q)))
}
onMounted(async () => {
  const id = Number(route.params.id)
  const r = await getPracticeRecord(id)
  record.value = r ?? null
  if (record.value) loadAllQuestions(record.value.subject)
})
</script>
